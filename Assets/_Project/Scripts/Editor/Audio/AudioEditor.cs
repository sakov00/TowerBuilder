...
namespace SimblendTools
{
    public class AudioEditor : EditorWindow
    {
        ...
        private void TrimAndFadeAudioClip()
        {
            ...
            // Get the original path of the audio clip
            string originalPath = AssetDatabase.GetAssetPath(audioClip);
            if (string.IsNullOrEmpty(originalPath))
            {
                Debug.LogError("Could not retrieve the path for the selected audio clip.");
                return;
            }

            // Get the directory and filename of the original audio
            string directory = Path.GetDirectoryName(originalPath);
            string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(originalPath);

            // Create a new path for the edited audio
            string newPath = Path.Combine(directory, fileNameWithoutExtension + "_EDITED.wav");
            ...
            SaveAsWav(trimmedClip, newPath);

            Debug.Log($"Trimmed and faded audio clip saved to {newPath}");
